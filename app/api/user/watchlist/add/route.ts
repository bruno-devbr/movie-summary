...
// Função que trata requisições POST para adicionar um item à watchlist do usuário
export async function POST(req: NextRequest) {
    try {
        ...
        // Chama a API para adicionar o item à watchlist do usuário
        await api.accountWatchlistUpdate(
            {
                id: account_id,
                media_id: rawBody.media_id,
                media_type: rawBody.media_type,
                watchlist: rawBody.watchlist,
                language: "pt-BR",
            },
            setHeaders()
        );
